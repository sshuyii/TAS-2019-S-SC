...
//		return ocean == null ? 0.0f : ocean.GetWaterHeightAtLocation(x, z);
        ...
        int layerMask = 1 << 4;
    //    layerMask = ~layerMask;
        if (Physics.Raycast(new Vector3(x, 100f, z), Vector3.down, out hit,200f, layerMask))
        {
          //  print(hit.point.y + hit.collider.name);
            return hit.point.y;
        }
        return -1000;
	//	return 0.0f;
	...
	private void OnDrawGizmos()
	{
            ...
            {
                Gizmos.DrawIcon(transform.position, "buoyancy_icon_linear.png", true);
            }
            else
            {
                Gizmos.DrawIcon(transform.position, "buoyancy_icon_gamma.png", true); 
            }
