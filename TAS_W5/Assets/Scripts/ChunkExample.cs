...
public class ChunkExample : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        borderVertices = new Vector3[CubeTreadmill.sizeSquare * 4 + 4];
        ...
        for (int z = 0; z <= sizeSquare; z++)
        {
            for (int x = 0; x <= sizeSquare; x++)
            {
                _verts[(z * (sizeSquare + 1)) + x] =
                    new Vector3(x,
                         5 * Perlin.Noise(
                            10 * ((float) x + transform.position.x) / sizeSquare, // * 10得到一个起伏更明显的terrain 
...
//            print("vertexIndexA = " + vertexIndexA);
//            print("vertexIndexB = " + vertexIndexB);
//
//            print("vertexIndexC = " + vertexIndexC);
...
//        for (int i = 0; i < _totalVertIndApplied; i++)
//        {
//            print("_normal = " + _normalsApplied[i]);
//        }
